...
public class DualCESEvaluation {
    ...
    private static void evaluateDualCESSnippet(String folder, List<String> keywords, List<String> querywords){
        try {
            File file = new File(folder + "/dualces-snippet.txt");
            if (!file.exists()) {
        ...
        String snippetStr = ReadFile.readString(folder + "/dualces-snippet.txt").get(1);
...
//        evaluateDualCESSnippet(baseFolder, keywords, keywords);
//    }
}
