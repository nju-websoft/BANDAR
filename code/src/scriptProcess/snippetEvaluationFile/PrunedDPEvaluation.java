...
public class PrunedDPEvaluation {
    ...
    private static void evaluatePrunedDPSnippet(String folder, List<String> keywords, List<String>querywords){
        try {
            File file = new File(folder + "/pruneddp-snippet.txt");
            if (!file.exists()) {
        ...
        String snippetStr = ReadFile.readString(folder + "/pruneddp-snippet.txt").get(1);
...
//        evaluatePrunedDPSnippet(baseFolder, keywords, keywords);
//    }
}
