...
public class TACEvaluation {
    ...
    private static void evaluateTACSnippet(String folder, List<String> keywords, List<String>querywords){
        try {
            File file = new File(folder + "/tac-snippet.txt");
            if (!file.exists()) {
        ...
        String snippetStr = ReadFile.readString(folder + "/tac-snippet.txt").get(1);
...
//        evaluateTACSnippet(baseFolder, keywords, keywords);
//    }
}
