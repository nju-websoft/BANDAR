...
public class PrunedDPSnippet{
	...
	public void createLabelIdIndex(String folder) {
		try {
			File file = new File(folder + "/nodeKwdIndex");
			if (file.exists()) {
				for (File f: file.listFiles()) {
					f.delete();
				}
			}
			Directory nodeKwdDir = FSDirectory.open(Paths.get(folder + "/nodeKwdIndex"));
			IndexWriter writer = new IndexWriter(nodeKwdDir, new IndexWriterConfig(new StemAnalyzer()));
			
			Set<Integer> nodeSet = new HashSet<>();
			for (List<Integer> iter: ReadFile.readInteger(folder + "/triple.txt", "\t")) {
				nodeSet.add(iter.get(0));
				nodeSet.add(iter.get(2));
			}
			...
			for (List<String> iter: ReadFile.readString(folder + "/uri_label.txt", "\t")) {
				...
				if(keyword.length() == 0)
					continue;
				if(!nodeSet.contains(Integer.parseInt(iter.get(0))))
					continue;
				Document doc = new Document();
				TextField keywordField = new TextField("keyword", keyword, Field.Store.YES);
				TextField idField = new TextField("id", iter.get(0), Field.Store.YES);
	...
	private void init() {
		for (List<String> iter: ReadFile.readString(resultFolder + "/indexes/uri_label.txt", "\t")) {
		...
		try {
			...
			IndexReader reader = DirectoryReader.open(FSDirectory.open(Paths.get(resultFolder + "/indexes/trpIndex")));
...
//				System.out.println("best changed to : "+best);
	...
	private List<Integer> getKeywordNodes(String keyword){
		...
		try {
			IndexSearcher searcher = new IndexSearcher(DirectoryReader.open(FSDirectory.open(Paths.get(resultFolder + "/indexes/nodeKwdIndex"))));
			QueryParser parser = new QueryParser("keyword", new StemAnalyzer());
