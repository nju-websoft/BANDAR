...
import beans.FullNode;
import beans.property;
import beans.typeClass;
...
public class IlluSnip {

	public Multigraph<Integer, DefaultWeightedEdge> findSnippet(String folder) {
		this.folder = folder;
	...
	public void create(){
		...
		try {
			for (List<Integer> iter: ReadFile.readInteger(folder + "/indexes/triple.txt", "\t")) {
				if(iter.get(0).equals(iter.get(2))) continue;
			...
			for (List<String> iter: ReadFile.readString(folder + "/indexes/uri_label.txt", "\t")) {
				String s = iter.get(2);
				labelID.put(Integer.parseInt(iter.get(0)), iter.get(2));
				if(s != null) {
					ERgraph.addVertex(s);
				}
				Ncount++;
