...
public class KSDEvaluation {

    private void evaluateKSDSnippet(int dataset, ArrayList<String>keywords, ArrayList<String>querywords){
        String keyword = keywords.get(0);
        for (int i = 1; i < keywords.size(); i++){
            keyword += " " + keywords.get(i);
        }
        Connection connection = new DBUtil().conn;
        String select = "select snippet from snippet where algorithm = 'KSD' and keyword = '" + keyword + "' and dataset_local_id = " + dataset + " limit 1";
        try {
    ...
    public static void main(String[] args){
        KSDEvaluation test = new KSDEvaluation();
        ...
        test.evaluateKSDSnippet(1, keywords, keywords);
